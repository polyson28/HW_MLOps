...
def _proto_to_python_matrix(proto_rows):
    ...
    return [[_proto_value_to_python(val) for val in row.values] for row in proto_rows]
...
class MLServiceServicer(ml_pb2_grpc.MLServiceServicer):
    ...
    def Train(self, request, context):
        ...
        try:
            ...
            hyperparams = (
                _string_dict_to_typed_dict(dict(request.hyperparams))
                if request.hyperparams
                else None
            )
            feature_types = (
                list(request.feature_types) if request.feature_types else None
            )
        ...
        try:
            ...
            for info in models_info:
                model_info = ml_pb2.ModelClassInfo(
                    ...
                    default_params={
                        k: str(v) for k, v in info["default_params"].items()
                    },
                    param_schema_json=json.dumps(
                        info["param_schema"], ensure_ascii=False
                    ),
                )
    ...
    def Predict(self, request, context):
        ...
        try:
            ...
            result = ml_core.predict(model_id=request.model_id, X=X)
            ...
            logger.info(
                f"gRPC Predict: успешно получено {len(result['predictions'])} предсказаний"
            )
            return response
    ...
    def DeleteModel(self, request, context):
        ...
        try:
            logger.info(
                f"gRPC DeleteModel: model_id={request.model_id}, hard={request.hard}"
            )

            ml_core.delete_model(model_id=request.model_id, hard=request.hard)

            delete_type = (
                "полностью удалена" if request.hard else "помечена как удалённая"
            )
            response = ml_pb2.DeleteResponse(
                model_id=request.model_id, message=f"Модель {delete_type}"
            )
    ...
    def HealthCheck(self, request, context):
        ...
        return ml_pb2.HealthResponse(status="ok", message="gRPC ML Service is running")
