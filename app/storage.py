...
class ModelStorage:
    ...
    """
    def __init__(self, base_dir: str | Path = "models_storage", db_filename: str = "models_db.json") -> None:
    ...
    def _load_db(self) -> None:
        ...
        if not self._db_path.exists():
            logger.info("Файл метаданных %s не найден, создаём пустую БД.", self._db_path)
            self._db = {}
            return
        ...
        try:
            with self._db_path.open("r", encoding="utf-8") as f:
                ...
                if not isinstance(data, dict):
                    raise RuntimeError("Файл метаданных имеет неверный формат (ожидается dict).")
                self._db = data
        except (OSError, json.JSONDecodeError) as exc:
            logger.exception("Ошибка при чтении файла метаданных %s", self._db_path)
            raise RuntimeError(f"Не удалось прочитать файл метаданных: {self._db_path}") from exc
    ...
    def _save_db(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"Не удалось сохранить файл метаданных: {self._db_path}") from exc
    ...
    def save_new_model(
        ...
        try:
            ...
            raise RuntimeError(f"Не удалось сохранить модель в файл: {model_path}") from exc
    ...
    def update_existing_model(
        ...
        with self._lock:
            ...
            try:
                ...
                raise RuntimeError(f"Не удалось обновить модель в файле: {model_path}") from exc
    ...
    def load_model(self, model_id: str) -> Any:
        ...
        try:
            ...
            raise RuntimeError(f"Не удалось загрузить модель из файла: {model_path}") from exc
        ...
        with self._lock:
            ...
            if model_path.exists():
                try:
                    ...
                    logger.warning("Не удалось удалить файл модели %s: %s", model_path, exc)
