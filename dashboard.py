...
st.title("Dashboard for training & monitoring ml-models")
st.markdown("---")
...
def train_model(
    ...
    """Обучить модель"""
    try:
        payload = {"model_class_key": model_class_key, "X": X, "y": y}
        if hyperparams:
...
def retrain_model(
    ...
    """Переобучить модель"""
    try:
        payload = {"model_id": model_id, "X": X, "y": y}
        if hyperparams:
            payload["hyperparams"] = hyperparams
        if feature_types:
            payload["feature_types"] = feature_types
...
def parse_csv_to_data(uploaded_file) -> Optional[pd.DataFrame]:
    """Загрузка CSV файла
    (костыль для тестирования данных)
    """
    try:
...
if page == "🏠 Главная":
    ...
    for model in available_models:
        with st.expander(f"**{model['display_name']}** (`{model['key']}`)"):
            ...
            for param, schema in model["param_schema"].items():
                st.write(
                    f"- `{param}` ({schema['type']}): {schema.get('description', 'Нет описания')}"
                )
    ...
    selected_model_name = st.selectbox(
        "Выберите класс модели", options=list(model_options.keys())
    )
    selected_model = model_options[selected_model_name]

    st.info(
        f"Выбрана модель: **{selected_model['display_name']}** (`{selected_model['key']}`)"
    )
    ...
    uploaded_file = st.file_uploader(
        "Загрузите CSV файл с обучающими данными", type=["csv"]
    )
    ...
    if uploaded_file:
        ...
        if df is not None:
            ...
            target_col = st.selectbox(
                "Выберите целевую переменную (target)", options=df.columns.tolist()
            )
            ...
            if specify_types:
                ...
                for col in feature_cols:
                    col_type = st.selectbox(
                        f"{col}", options=["numeric", "categorical"], key=f"type_{col}"
                    )
            ...
            if st.button("🚀 Обучить модель", type="primary"):
                with st.spinner("Обучение модели..."):
                    ...
                    if result:
                        st.success(
                            f"✅ Модель успешно обучена! ID: `{result['model_id']}`"
                        )
    ...
    if change_hyperparams and model_info:
        ...
        available_models = get_available_models()
        model_schema = next(
            (m for m in available_models if m["key"] == model_info["model_class_key"]),
            None,
        )
    ...
    if uploaded_file:
        ...
        if df is not None:
            ...
            target_col = st.selectbox(
                "Выберите целевую переменную", options=df.columns.tolist()
            )
    ...
    if st.button("🗑️ Удалить модель", type="primary"):
        if delete_model(selected_model_id, hard=hard_delete):
            delete_type = (
                "полностью удалена" if hard_delete else "помечена как удалённая"
            )
