...
- trainmodel и retrainmodel дополнительно выполняют версионирование датасета обучения через DVC (сохранение X/y в parquet → dvc add → dvc push в Minio).
...
### app/dvc_datasets.py
- Модуль версии датасетов обучения:
хранит локальный DVC-репозиторий и датасеты 
- при обучении выполняет dvc init/remote (если нужно), затем dvc add и dvc push в Minio (S3)
- в логах отображаются все DVC-команды и их вывод
- в Minio DVC хранит данные как кэш по хешу 
...
- minio: контейнер S3-хранилища (Minio) на 9000/9001.
- minio-init: контейнер инициализации (создание bucket mlops).
...
7) ml_core сохраняет датасет обучения в .parquet и версионирует его через DVC: dvc add + dvc push в Minio (S3).

8) ml_core обучает модель, при наличии score вычисляет метрику на обучающем наборе, подготавливает метаданные и передаёт модель в storage для сохранения .pkl и JSON-«БД».

9) storage атомарно пишет снапшот модели (joblib .pkl), обновляет JSON с метаданными (id, класс, гиперпараметры, статус, timestamps, метрики) под блокировкой, и возвращает ModelMetadata.

10) Функция ml_core.train_model() возвращает метаданные в REST-слой; эндпоинт формирует TrainResponse (id, класс, гиперпараметры, метрики, message) и устанавливает статус 201.

11) FastAPI сериализует ответ в JSON и отправляет его Streamlit-приложению; дэшборд получает model_id, отображает успешный результат и сохраняет id в состоянии для дальнейших действий (например, Predict/ Retrain/ List).

12) Параллельно или по кнопке «Обновить», Streamlit может обратиться к gRPC ListTrainedModels/GetModelInfo, чтобы быстрее подтянуть список и детальные метаданные обученных моделей; gRPC-сервер конвертирует protobuf <-> Python, вызывает те же функции ml_core и возвращает данные с корректными статусами.

13) Для инференса пользователь в Streamlit выбирает сохранённый model_id и отправляет HTTP POST /predict с X; FastAPI вызывает ml_core.predict, который загружает модель из storage и возвращает predictions.

14) Streamlit показывает результаты предсказаний и метрики.

## Точка входа / как запустить
### Запуск
...
## Тестирование 
Добавили тесты, которые лежат в `tests/`. Они проверяют корректную инициализацию DVC-репозитория и настройку remote Minio и корректную работу функции сохранения и версионирования датасета
Для изоляции тестов используются:
- pytest
- фикстура monkeypatch для подмены переменных окружения и вызовов subprocess.run
- временные директории tmp_path, предоставляемые pytest

Для упрощения типовых операций добавлен Makefile. Доступные команды:
- make docker-push IMAGE=<dockerhub_user>/<repo>:<tag>
- make test
- make lint

Первая команда сбирает Docker-образ в корне проекта и отправляет его в DockerHub под указанным тегом. Важно чтобы сработал push надо чтобы образ уже был создан в dockehub а также необходимо сделать docker login. Вторая команда запускает описанные выше тесты, а третья проверяет форматирование проекта.
